...
from src.build_hdbscan_temporal_feature_view import build_hdbscan_temporal_feature_view, export_hdbscan_temporal_feature_view
from src.build_temporal_windows import build_temporal_windows, export_temporal_windows, load_windowing_policy
from src.scenario_engine import build_scenario_events
from src.scenario_repository import load_training_scenarios
...
MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "correlation_engine_manifest.json"
METRICS_PATH = PROJECT_ROOT / "artifacts" / "metrics" / "hdbscan_temporal_training_summary_v0_1.json"
...
PRIORITY_FEATURE_KEYS = (
    ...
    "iforest_score_orders",
    "iforest_score_checkout",
    ...
    "lag_payments_to_redis",
    "lag_redis_to_checkout",
    "service_degradation_order_encoded",
...
def ensure_training_inputs() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    training_events = build_events_from_profiles(load_training_scenarios(), start_anchor=datetime(2026, 1, 1, tzinfo=timezone.utc))
    windows = build_temporal_windows(training_events, policy=load_windowing_policy())
    export_temporal_windows(windows)
    feature_rows = build_hdbscan_temporal_feature_view(windows)
    export_hdbscan_temporal_feature_view(feature_rows)
...
def train_baseline() -> TrainingResult:
    ...
    metrics = {
        "schema_version": "hdbscan_temporal_training_summary_v0_1",
    ...
    manifest = {
        "schema_version": "correlation_engine_manifest_v0_1",
        "engine_name": "hdbscan_temporal",
        "engine_version": "baseline_v0_1",
...
def main() -> int:
    ...
    print(
        "HDBSCAN temporal baseline ready: "
